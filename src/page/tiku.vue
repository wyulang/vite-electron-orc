<template>
  <div class="w-all flex pl15 h-all fd-c pr5">
    ...
    <div class="mt10 nodarg flex fd-c">
      <textarea @input="changeInput" class="w-all bc-t h-150 ipt ipt-auto" v-model="info.words" name="" id="" cols="30" rows="10"></textarea>
      <div class="flex ai-c mt10">
        年级<input v-model="info.grade" class="ipt ipt-samll w-90 ra-4 b-d pp10" type="text">
        单元<input v-model="info.unit" class="ipt ipt-samll w-90 ml10 ra-4 b-d pp10" type="text">
        上下册<input v-model="info.volume" class="ipt ipt-samll w-90 ml10 ra-4 b-d pp10" type="text">
        <div @click="btnWord()" v-dubclick class="btn hand btn-small btn-primary ml10 w-120 fc-fff pl20 pr20  ra-4">保存</div>
      </div>
    </div>
    <div class="h-15"></div>
    <div class="flex-1 h-all w-all">
      <scroll>
        <div class="rel drag-select">
          ...
          <!-- @dblclick="isAdd=true" -->
          <div class="abs at0 al0 h-all flex fd-c zi-130 w-all ar0">
            <!-- v-copy="item.text" -->
            <!-- <div class="abs b-e pp3 fs-14 drag-select" :style="item.style" v-for="item in captureInfo.text">{{item.text}}</div> -->
            <!-- <div class="w-all b-e pp3 fs-14 drag-select" v-for="item in info.serchlist">{{item,word}},{{item,phonetic}}</div> -->
          </div>
        </div>
      </scroll>
    </div>
  ...
  <drawer width="1000" v-model="isAdd">
    <template v-slot:btn>
      <div class="flex nodarg flex-1 ai-c">
        ...
        <tooltip click class="ml10">
          ...
          <template v-slot:content>
            <div class="w-500 pp20 flex fw">
              <span class="w-40 h-40 flex ai-c jc-c fs-20 mb10 hand mr10 b-e" v-copy="item" v-for="item in info.fuhao">{{item}}</span>
            </div>
          </template>
        </tooltip>
        <span class="flex-1"></span>
        <div @click="isAdd=false" class="btn hand btn-small ml20 btn-default ra-4">取消</div>
...
info.words=`,exam:[${info.words.replace(/\n/g,',').replace(/,+/g, ',').split(',').map(v=>`'${v}'`)}]`
  // info.words = info.words.replace(/[\s*]/g, '').replace(/[\u4e00-\u9fa5]/g, '-').replace(/[^\w]/g, ',').replace(/,+/g, ',');
}

function btnWord() {
  if (!info.grade || !info.unit || !info.volume) {
    yy.msg.error('不为空')
    return;
  }
  let sql = "UPDATE dictionary t JOIN (VALUES "
  info.words.split(',').forEach((item, i) => {
    sql = sql + `${!i ? '' : ','} ROW('${item}', ${info.grade},${info.unit},${info.volume})`
  })
  sql = sql + ' ) AS v(word, grade,unit,volume) ON t.word = v.word SET t.grade = v.grade,t.unit = v.unit,t.volume = v.volume;'
  yy.spinner.show()
  ipcRenderer.invoke('words', { sql: sql }).then(res => {
    yy.spinner.close();
  })
}

function changeDic() {
  info.serchlist = info.dic.filter(v => v.word.includes(info.kemu))
}
...
function toCapture() {
  ...
  ipcRenderer.invoke('capture-image', { orc: true }).then(res => {
    console.log(res)
    yy.spinner.close()
    if (res.text.code == 100) {
      ...
      info.words = res.text.data.map(v => v.text).toString();
