...
class webapi {
  ...
  // }
  config(route: string, data: any = undefined, method: string, axiosConfig: any = {}) {
    ...
    Object.keys(axiosConfig).forEach(v => {
      if (v == "isUrl") {
        ...
        url = this.getDomainApi(axiosConfig[v]).concat(route)
      }
    })
  ...
  request(url: string, data: any = undefined, method: string, header: any, responseType: string = "json", withCredentials: string = "", isQs: string = "") {
    ...
    return axios.request(soucrs).catch(res => {
      if (!res.response) {
        this.getMessage({ code: 404, data: res.config.url }, 'catch');
        // return { data: { code: -1 } }
      }
      else if (res.response.status != 200) {
        if (res.request.responseType == "blob") {
          this.getMessage({ code: res.response.status, data: res.response.statusText }, 'catch');
          // return { data: { code: -1 } }
        } else {
          this.getMessage({ code: res.response.status, data: res.response.data }, 'catch');
          // return { data: { code: -1, data: res.response.data } }
        }
      } else {
      ...
      if (result?.data && String(result?.data.code)) {
        return result.data;
      } else {
  ...
  get(url: any, data: any = undefined, config = {}) {
    return this.config(url, data, 'get', config)
  }
  post(url: any, data: any = undefined, config = {}) {
    return this.config(url, data, 'post', config);
  }
  put(url: any, data: any = undefined, config = {}) {
    return this.config(url, data, 'PUT', config);
  }
  delete(url: any, data: any = undefined, config = {}) {
