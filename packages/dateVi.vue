<template>
  <div ref="dateMain" :class="bodyClass" class="b-d ra-4 noselect _date rel w-all">
    <div id="dateInput" @mouseover="mainOver" @mouseout="mainOut" @click="initDatePop" class="flex ai-c ra-4 hidden pp3 h-all w-all">
      <input :class="{'inputover':sDay.isOver}" ariaValueText="start" :value="currStartDate" :placeholder="twin?startPlaceholder:placeholder" @mousemove="movesDay('star')" @input="inputStartValue" @mouseout="outDay('star')" class="pl8 ra-4 pr8 fs-init flex-1 w-all h-all" type="text">
      <span v-if="twin">{{separator}}</span>
      <input v-if="twin" :class="{'inputover':eDay.isOver}" ariaValueText="end" :value="endStartDate" :placeholder="endPlaceholder" @mousemove="movesDay('end')" @mouseout="outDay('end')" @input="inputStartValue" class="pl8 ra-4 pr8 fs-init flex-1 w-all h-all" type="text">
    ...
    <div v-if="info.visible" ref="layDate" :style="bodyDate" class="abs bc-fff zi-8888 flex-line sha-6">
      ...
      <!-- 结束时间 -->
      <div v-if="twin" style="width:274px" class="bl-d">
        <!-- 分页 -->
        <div class="w-all mt4 flex ai-c jc-b">
          <!-- 左分页 -->
          <div class="flex ai-c">
      ...
      <!-- 年月选择 -->
      <div v-if="info.isYear" style="width:272px" :class="{'al0':info.currYearType,'ar0':!info.currYearType}" class="abs at0 flex ab0 fd-c ra-4 bc-fff">
        ...
        <div class="flex flex-1 hidden">
          <div id="yearScoll" class="w-90 h-all auto noscoll">
            <div @click="onSetYear(item)" class="h-32 flex ai-c br-e jc-c hand" :class="{'bc-f6 fc-primary ':(item==(info.currYearType?sDay.nextYear:eDay.nextYear))}" v-for="item in years">{{item}}</div>
          </div>
          <div id="monthScoll" class="w-90 h-all auto noscoll">
            <div @click="onSetMonth(item,i)" :class="{'bc-f6 fc-primary ':(i==(info.currYearType?sDay.nextMonth:eDay.nextMonth))}" class="h-32 flex ai-c fs-14 br-e jc-c hand" v-for="item,i in months">{{item}}</div>
          </div>
        </div>
      </div>

      <!-- 时分秒选择 -->
      <div v-if="info.isTime" style="width:272px" :class="{'al0':info.currYearType,'ar0':!info.currYearType}" class="abs at0 flex ab0 fd-c ra-4 bc-fff">
        <div class="pt10 pb10 bb-e">
        ...
        <div class="flex flex-1 hidden">
          <div id="hourScoll" class="flex-1 h-all auto noscoll">
            <div @click="onSetTime(item,'hourScoll')" class="h-32 flex ai-c br-e jc-c hand" :class="{'bc-f6 fc-primary ':(item==(info.currYearType?sDay.hour:eDay.hour))}" v-for="item in hours">{{item}}</div>
          </div>
          <div id="minuteScoll" class="flex-1 h-all auto noscoll">
            <div @click="onSetTime(item,'minuteScoll')" class="h-32 flex ai-c br-e jc-c hand" :class="{'bc-f6 fc-primary ':(item==(info.currYearType?sDay.minute:eDay.minute))}" v-for="item in minutes">{{item}}</div>
          </div>
          <div id="secrodScoll" class="flex-1 h-all auto noscoll">
            <div @click="onSetTime(item,'secrodScoll')" class="h-32 flex ai-c br-e jc-c hand" :class="{'bc-f6 fc-primary ':(item==(info.currYearType?sDay.second:eDay.second))}" v-for="item in minutes">{{item}}</div>
          </div>
        </div>
      </div>
    </div>
...
const end = defineModel('end', { type: [String, Number], default: "" });
//change 事件
...
const years = Array.from(new Array(150).keys()).map(v => 1919 + v);
