...
let ps = defineProps({
  ...
  zindex: { type: [String, Number], default: 9111 },
  // top高度
  ...
  // pop输入框验正文案
  popMsg: { type: String, default: '内容不为空' },
  // 显示时长
...
// 手机端
let isPhone = /(iPhone|iOS|Android)/i.test(navigator.userAgent);
let popValue = ref("");
let timer: any;
let visible = ref(true);
let popValid = ref(false);
...
const isMessage = computed(() => {
  if (ps.type == 'phone') {
    return 1
  } else if (['success', 'warning', 'info', 'error'].includes(ps.type)) {
    return 2
  } else if (ps.type == 'confirm') {
    return 3
  } else if (ps.type == 'pop') {
    return 4
  }
...
let messageStype = computed(() => {
  let style: any = {}
  if (isMessage.value == 1) {
    if (ps.placement == 'top') {
      style.top = `${ps.offset}px`;
      style.left = "50%";
      style.transform = "translateX(-50%)"
    } else if (ps.placement == 'middle') {
      style.top = "50%";
      style.left = "50%";
...
onMounted(()=>{
  if (['success', 'warning', 'info', 'error', 'phone'].includes(ps.type)) {
    startTimer()
  }
})
...
const messageClass = computed(() => {
  return {
    ...
    [ps.className]: ps.className,
    'success': ps.type == 'success',
    'warning': ps.type == 'warning',
    'info': ps.type == 'info',
    'error': ps.type == 'error',
    'phone': ps.type == 'phone',
    'absl': isMessage.value == 2,
    'messageFadeInDown': visible.value,
    'messageFadeOutUp': !visible.value,
  }
})
...
function btnSave(type) {
  if (type == 'pop') {
    if (!popValue.value) {
      ...
      ps.close && ps.close({ type, value: popValue.value });
      ps.onClose(ps.id);
      ps.destroy();
    }
  } else {
    ps.close && ps.close(type);
    ps.onClose(ps.id);
    ps.destroy();
  }
}
