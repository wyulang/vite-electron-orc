<template>
  <div ref="main" :class="skins" class="flex ai-c rel fs-22 ra-4 jc-c">
    <span class="fc-ccc fs-14 abs al6 abst">{{suftext}}</span>
    <i @click="onSub" v-if="direction=='default'" :class="{...btnclass,'ra-5':skin==2}" class="h-all hand noselect flex ai-c jc-c">-</i>
    <input :value="modelValue" :max="max" :min="min" @input="onInput" @mousewheel.prevent="onMouseWhell" :placeholder="placeholder" :class="{' ra-5':skin==2,[inputClass]:true}" class="flex-1 bc-t h-all centers w-all" checked type="text">
...
const ps = defineProps({
  class: { type: String, default: "h-35 w-150" },
  // 输入框样式
  inputClass: { type: String, default: "fs-14" },
  //样式 1为一体，2为分开，3为right
  skin: { type: String, default: "1" },
  ...
  direction: { type: String, default: 'default' }
})

const heights = ref(0);
const main = ref(null);
const isApp = /(iPhone|iPad|iPod|iOS|Android)/i.test(navigator.userAgent);
const emit = defineEmits(["update:modelValue", "change"]);

const skins = computed(() => {
  let c = 'number-' + ps.skin;
...
const btnclass = computed(() => {
  let c = 'wi-' + heights.value;
  if (isApp) {
    c = 'wi-' + parseInt(heights.value / 2 + 5);
  }
  return {
    [c]: true
  }
...
function onAdd() {
  if (ps.max && ps.modelValue > parseFloat(ps.max)) {
    changeValue(ps.max)
    return;
  } else {
    let val = parseFloat(ps.modelValue || 0) * 10000 + parseFloat(ps.step) * 10000;
    if (val / 10000 > parseFloat(ps.max)) {
      val = parseFloat(ps.max) * 10000
    }
    changeValue(val / 10000)
  }
}
...
function onSub() {
  if (ps.min && parseFloat(ps.modelValue) < parseFloat(ps.min)) {
    changeValue(ps.min)
    return;
  } else {
    let val = parseFloat(ps.modelValue || 0) * 10000 - parseFloat(ps.step) * 10000;
    if (val / 10000 < parseFloat(ps.min)) {
      val = parseFloat(ps.min) * 10000
    }
    changeValue(val / 10000)
  }
}
...
function cheight() {
  heights.value = main.value.offsetHeight;
}
